 ...
 * - ALLOWED_URI_REGEXP で href スキームを https?/mailto のみに制限
 * - target 属性は許可しない。DOMPurify v3 は anti-tabnabbing でデフォルトで剥がすが、
 *   明示的に ALLOWED_ATTR から外して意図を明確化する。新規タブが必要になっても、
 *   表示側で onClick 経由で開く設計にし、サニタイザで target を扱わない。
 */
